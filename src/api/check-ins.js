...
async function getCheckInIds(orgId) {
	...
	checkIns.forEach((key) => {
		...
		getOrCreate(checkInsByEventId, eventId, () => new Set()).add(participantId);
		getOrCreate(checkInsByParticipantId, participantId, () => new Set()).add(
			eventId
		);
	});
...
export async function getEventCheckIns(orgId, eventId) {
	const participantIds =
		(await getCheckInIds(orgId)).checkInsByEventId.get(eventId) || [];
	const checkInPromises = [...participantIds].map(async (participantId) => {
...
export async function getParticipantCheckIns(orgId, participantId) {
	...
	const eventIds =
		(await getCheckInIds(orgId)).checkInsByParticipantId.get(participantId) ||
		[];
	const checkInPromises = [...eventIds].map(async (eventId) => {
